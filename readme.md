...
Although it's best practice to show these kind of notifications, it's also possible to turn it off by setting `updateNotification` to `false` in the workflow's `package.json` file.
